...
function AdminMenu() {
  ...
  const handleDelete = async (id: string) => {
    try { setActionError(null); await deleteItem(id); }
    catch (e: unknown) { setActionError(e instanceof Error ? e.message : "Failed to delete"); }
  };

  const handleToggleStock = async (id: string) => { try { await toggleOutOfStock(id); } catch { /**/ } };
  const handleToggleFeatured = async (id: string) => { try { await toggleFeatured(id); } catch { /**/ } };
...
// ─── Sub-component: table + mobile cards ──────────────────────────────────────
type TableProps = {
...
function ItemTable({ filtered, categories, onEdit, onDelete, onToggleStock, onToggleFeatured }: TableProps) {
              ...
              <tr key={item.id} className={`hover:bg-secondary/20 transition ${item.outOfStock ? "opacity-60" : ""}`}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {item.featured && <span className="text-xs px-2 py-0.5 rounded-full bg-amber-400/10 text-amber-400 border border-amber-400/20">⭐ Featured</span>}
                  </div>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => onToggleStock(item.id)} title={item.outOfStock ? "Mark in stock" : "Mark out of stock"}
                    ...
                    <button onClick={() => onEdit(item)} className="p-1.5 rounded-md text-muted-foreground hover:text-saffron hover:bg-saffron/10 transition">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => onDelete(item)} className="p-1.5 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition">
        ...
        {filtered.map((item) => (
          <div key={item.id} className={`p-4 ${item.outOfStock ? "opacity-60" : ""}`}>
            ...
            <div className="flex items-center gap-2 flex-wrap mb-3">
              ...
              {item.outOfStock && <span className="text-xs px-2 py-0.5 rounded-full bg-destructive/15 text-destructive border border-destructive/20">Out of Stock</span>}
              {item.featured && <span className="text-xs px-2 py-0.5 rounded-full bg-amber-400/10 text-amber-400 border border-amber-400/20">⭐ Featured</span>}
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => onToggleStock(item.id)}
                className={`flex-1 py-1.5 rounded-lg text-xs border transition ${item.outOfStock ? "border-green-500/30 text-green-400 hover:bg-green-500/10" : "border-destructive/30 text-destructive hover:bg-destructive/10"}`}>
              ...
              <button onClick={() => onEdit(item)} className="py-1.5 px-3 rounded-lg text-xs border border-border text-muted-foreground hover:text-saffron transition">
                <Pencil className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => onDelete(item)} className="py-1.5 px-3 rounded-lg text-xs border border-border text-muted-foreground hover:text-destructive transition">
