...
function AdminMenu() {
  ...
  const { items, categories, addItem, updateItem, deleteItem, toggleOutOfStock, toggleFeatured } =
    useMenuStore();
  ...
  const hasActiveFilters =
    filters.search || filters.category !== "all" || filters.diet !== "all" || filters.status !== "all";

  const clearFilters = () =>
    setFilters({ search: "", category: "all", diet: "all", status: "all" });
        ...
        <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="ml-auto flex items-center gap-1 text-saffron hover:underline"
            >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  ...
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">Category</th>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">Diet</th>
                  ...
                  <tr key={item.id} className={`hover:bg-secondary/20 transition ${item.outOfStock ? "opacity-60" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {item.outOfStock ? (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-destructive/15 text-destructive border border-destructive/20">
                            Out of Stock
                          </span>
                        ) : (
            ...
            {filtered.map((item) => (
              <div key={item.id} className={`p-4 ${item.outOfStock ? "opacity-60" : ""}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  ...
                  {item.featured && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-amber-400/10 text-amber-400 border border-amber-400/20">⭐ Featured</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    {item.outOfStock ? "✅ Mark In Stock" : "🚫 Mark OOS"}
                  </button>
                  <button
                    onClick={() => toggleFeatured(item.id)}
                    className={`py-1.5 px-3 rounded-lg text-xs border transition ${
                      item.featured
                  ...
                  <button
                    onClick={() => { setEditItem(item); setModalOpen(true); }}
                    className="py-1.5 px-3 rounded-lg text-xs border border-border text-muted-foreground hover:text-saffron hover:border-saffron/30 transition"
                  >
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button
                    onClick={() => setDeleteTarget(item)}
                    className="py-1.5 px-3 rounded-lg text-xs border border-border text-muted-foreground hover:text-destructive hover:border-destructive/30 transition"
                  >
