                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-2 flex-shrink-0">
                      {cat.count} items
                      {cat.outOfStock > 0 && (
                        <span className="text-destructive ml-1">· {cat.outOfStock} OOS</span>
                      )}
                    </span>
                  </div>
                  <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-saffron to-ember rounded-full transition-all"
            ...
            {recentItems.map((item) => (
              ...
              >
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                  item.diet === "veg" ? "bg-green-500" :
                  item.diet === "nonveg" ? "bg-red-500" : "bg-amber-400"
                }`} />
                <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  {item.outOfStock && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-destructive/15 text-destructive border border-destructive/20">
                      OOS
                    </span>
                  )}
                  {item.featured && (
        ...
        <div className="mt-6 flex items-center gap-3 p-4 rounded-xl bg-destructive/10 border border-destructive/30">
          ...
          <div className="flex-1">
            <span className="text-sm font-medium text-destructive">
              {outOfStock} item{outOfStock > 1 ? "s are" : " is"} currently out of stock.
            </span>
            <span className="text-sm text-muted-foreground ml-1">
              Update availability in the menu section.
            </span>
          </div>
          <Link
            to="/admin/menu"
