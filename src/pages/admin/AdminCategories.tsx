...
export default function AdminCategories({ navigate }: Props) {
  ...
  const handleDelete = async (id: string) => {
    try { await deleteCategory(id); } catch (e: unknown) { setActionError(e instanceof Error ? e.message : "Failed to delete"); }
  };
            ...
            return (
              <div key={cat.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-4 hover:border-saffron/30 transition">
                ...
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => reorderCategory(cat.id, "up")} disabled={idx === 0}
                  ...
                  <button onClick={() => reorderCategory(cat.id, "down")} disabled={idx === sorted.length - 1}
                    className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition disabled:opacity-20 disabled:cursor-not-allowed">
